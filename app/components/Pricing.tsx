...
type Tier = Dict["pricing"]["tiers"][number];
...
function TierCard({
  ...
  // Tapping toggles the same "hovered" look a mouse hover gives — CSS
  // :hover/:active never reliably fire on mobile for a plain div, so this
  // is driven by real state instead of a pseudo-class.
  const [pressed, setPressed] = useState(false);

  return (
    <GlareHover
      glareColor="#0cc0df"
      glareOpacity={featured ? 0.35 : 0.2}
      glareSize={220}
      ...
      className={`flex h-full flex-col rounded-lg border bg-card p-6 transition-all duration-300 hover:scale-[1.02] sm:p-8 ${
        ...
        featured
          ...
          : `border-line hover:border-accent-deep ${pressed ? "border-accent-deep" : ""}`
      }`}
    >
      <div className="flex items-center justify-between">
        <h3 className="display-type text-xl font-bold">{tier.name}</h3>
        {featured && (
      ...
      <p className="mt-4">
        <span className="display-type text-4xl font-bold">{tier.price}</span>
        <span className="ml-1 font-mono text-xs text-muted">{tier.unit}</span>
      </p>
      <p className="mt-3 text-sm text-muted">{tier.blurb}</p>
      <ul className="mt-5 grow space-y-2 border-t border-line pt-5">
        {tier.features.map((feature) => (
          <li key={feature} className="flex items-baseline gap-2 text-sm text-fg/85">
        ...
        <div className="mt-10 grid gap-4 md:grid-cols-3">
          {t.pricing.tiers.map((tier, i) => (
            <Reveal key={tier.name} delay={i * 100} className="h-full">
              <TierCard
                tier={tier}
                featured={tierMeta[i].featured}
                featuredTag={t.pricing.featuredTag}
                ctaHref={waLink(lang)}
              />
            </Reveal>
          ))}
